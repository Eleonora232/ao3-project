...
# Define the new markdown and code cells to append
new_cells = [
    {
        "cell_type": "markdown",
        "metadata": {},
        "source": [
            ...
            "3. Calculate overall fandom baselines and compare them to the bridge authors' works using the active `TARGET_METRIC` (e.g., Hits, Kudos, or Comments)."
        ]
    },
    {
        "cell_type": "code",
        ...
        "source": [
            ...
            "marvel_bridge = marvel_df[marvel_df['author_str'].isin(bridge_authors)]\n",
            "teen_wolf_bridge = teen_wolf_df[teen_wolf_df['author_str'].isin(bridge_authors)]\n",
            "\n",
            "# 5. Calculate overall baselines for TARGET_METRIC\n",
            ...
            "\n",
            "# 6. Calculate bridge author work statistics for TARGET_METRIC\n",
            ...
            "print(\"=\" * 60)\n",
            "\n",
            "# 8. Render a premium comparison visualization\n",
...
# Ensure we don't append multiple times by checking for section title
notebook_str = json.dumps(nb)
if "7. Deep Dive: Marvel (Mega-Fandom) vs. Teen Wolf" not in notebook_str:
    nb["cells"].extend(new_cells)
    with open(notebook_path, "w") as f:
        json.dump(nb, f, indent=1)
    print("New cells appended successfully to popularity_analysis.ipynb!")
else:
    print("Deep Dive section is already present in popularity_analysis.ipynb.")
